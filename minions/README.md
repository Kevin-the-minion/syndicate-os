...
**OpenClaw** (Kevin & co). Different harnesses = different architectures =
...
`personas/` has starter identity files (grunt = executor, scribe = recorder,
skeptic = challenger). Customize freely — the production fleet gave each
minion a real personality (SOUL-style) and it made them better collaborators.
